...
int papermodel::addCrease( int lcrs_idx, int rcrs_idx, double slen, double dlen )
{
	...
	if( lcrs_idx <= 0 ){
		...
		for( int j=lcrs_idx; j<lcrcnt; j++ ){
			...
			if( slen>0.0 ){
				...
				if( flg_postproc_type==PPTYPE_MOVECP1 && j == -hCrs_cridx && 0 <= hCrs_cpidx && hCrs_cpidx < CCNT )
				{
					ret = optRulLen_SplineCP1( c0, c1, hCrs_cpidx, hCrs_cpdx, hCrs_cpdy );
				}
				else if( flg_postproc_type==PPTYPE_OPTIMIZE && j == -flg_rectifyCrs )
				{
					ret = optRulLen_SplineCP( c0, c1 );
				}
				else
				{
					c0->init_rlflg();
			...
			c1->setL2R( c0 );
			c1->calcTN2d();
			c1->calcTNB( flg_rectifyT, c1->Xsidx, c1->Xeidx );
			c1->calcLeft();
			//c1->calcLeft0(); // no folding (for debug)
		} // j
	}
	...
	if( rcrs_idx <= 0 ){
		...
		for( int j=rcrs_idx; j<rcrcnt; j++ ){
			...
			c1->calcTNB( flg_rectifyT, c1->Xsidx, c1->Xeidx );
			c1->calcRight();
			//c1->calcRight0(); // no folding (for debug)
		} // j
	}
end:
	return ret;
